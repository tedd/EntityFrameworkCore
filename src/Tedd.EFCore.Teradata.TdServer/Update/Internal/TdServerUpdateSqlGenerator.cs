...
namespace Tedd.EFCore.Teradata.TdServer.Update.Internal
{
    /// <summary>
    ///     <para>
    ...
    ///     </para>
    /// </summary>
    public class TdServerUpdateSqlGenerator : UpdateSqlGenerator, ITdServerUpdateSqlGenerator
    {
        /// <summary>
        ///     This is an internal API that supports the Entity Framework Core infrastructure and not subject to
        ///     the same compatibility standards as public APIs. It may be changed or removed without notice in
        ...
        ///     doing so can result in application failures when updating to a new Entity Framework Core release.
        /// </summary>
        public TdServerUpdateSqlGenerator(
            ...
            int commandPosition)
        {
            if (modificationCommands.Count == 1
                && modificationCommands[0].ColumnModifications.All(
                    o =>
                        !o.IsKey
                        || !o.IsRead
                        || o.Property?.GetTdServerValueGenerationStrategy() == TdServerValueGenerationStrategy.IdentityColumn))
            {
            ...
            var nonIdentityOperations = modificationCommands[0].ColumnModifications
                .Where(o => o.Property?.GetTdServerValueGenerationStrategy() != TdServerValueGenerationStrategy.IdentityColumn)
                .ToList();
            ...
            if (modificationCommands[0].Entries.SelectMany(e => e.EntityType.GetAllBaseTypesInclusive())
                .Any(e => e.GetTdServerIsMemoryOptimized()))
            {
