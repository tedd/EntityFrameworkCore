...
{
    public class TdServerDatabaseCreatorTest
    {
        [ConditionalFact]
        ...
        private async Task Create_checks_for_existence_and_retries_until_it_passes(int errorNumber, bool async)
        {
            var customServices = new ServiceCollection()
                .AddScoped<ITdServerConnection, FakeTdServerConnection>()
            ...
            var contextServices = TdServerTestHelpers.Instance.CreateContextServices(customServices);

            var connection = (FakeTdServerConnection)contextServices.GetRequiredService<ITdServerConnection>();

            connection.ErrorNumber = errorNumber;
            connection.FailureCount = 2;

            var creator = (TdServerDatabaseCreator)contextServices.GetRequiredService<IRelationalDatabaseCreator>();
        ...
        private async Task Create_checks_for_existence_and_ultimately_gives_up_waiting_test(bool async)
        {
            ...
            if (async)
            {
                await Assert.ThrowsAsync<TdException>(() => creator.CreateAsync());
            }
            else
            {
                Assert.Throws<TdException>(() => creator.Create());
            }
        }

        private class FakeTdServerConnection : TdServerConnection
        {
            private readonly IDbContextOptions _options;

            public FakeTdServerConnection(IDbContextOptions options, RelationalConnectionDependencies dependencies)
                : base(dependencies)
            {
                _options = options;
            }
            ...
            public override bool Open(bool errorsExpected = false)
            {
                if (++OpenCount < FailureCount)
                {
                    ...
                    throw TdExceptionFactory.CreateTdException(ErrorNumber);
                }
            ...
            public override ITdServerConnection CreateMasterConnection()
                => new FakeTdServerConnection(_options, Dependencies);
        }
